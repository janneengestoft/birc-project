...
### Define our input files for example
...
### Input for males chromosome X
chunkfile<-"../steps/finestructure/test_run_linked.chunkcounts.out" ## chromopainter chunkcounts file
mcmcfile<-"../steps/finestructure/test_run_linked_mcmc.xml" ## finestructure mcmc file
treefile<-"../steps/finestructure/test_run/stage4/test_run_linked_tree.xml" ## finestructure tree file



## Additional files that you can extract from finestructure
mappopchunkfile<-"males_chrX.mapstate.csv" # population-by-population chunkcount file for the populations used in the MAP (i.e tree)
system( paste("fs fs -X -Y -e X2",chunkfile,treefile,mappopchunkfile) )
meancoincidencefile<-"males_chrX.meancoincidence.csv" # pairwise coincidence, .i.e. proportion of MCMC files where individuals are found in the same 
...
pdf(file="Males_chrX_SimpleFullDendrogram.pdf",height=6,width=14)
...
pdf(file="Males_chrX_SimplePopulationDendrogram.pdf",height=6,width=12)
par(mar=c(8,2,2,2),cex=0.8)
# plot.dendrogram(popdendclear,horiz=FALSE,nodePar=list(cex=0,lab.cex=1.2,las=2),edgePar=list(p.lwd=0,t.srt=0,t.off=0.3),yaxt="n",height=0.5,dLeaf=0.2)
dev.off()
...
pdf(file="Males_chrX_SimplePairwiseCoincidence.pdf",height=12,width=12)
...
pdf(file="Males_chrX_SimpleCoancestry.pdf",height=12,width=12)
...
tmpmat[tmpmat>tmatmax]<-tmatmax # 
pdf(file="Males_chrX_PopAveragedCoancestry.pdf",height=12,width=12)
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=0.6,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=0.8))
dev.off()
...
pdf(file="Males_chrX_Coancestry2.pdf",height=12,width=12)
...
pdf("Males_chrX_PCA.pdf",height=16,width=12)
par(mfrow=c(4,3))
...
png(file="Males_chrX_Haplotype.png",height=768,width=1024)
